    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-surface border border-outline-variant p-4.5 rounded-xl shadow-sm hover:border-primary/30 transition-all duration-200"
          >
            <span className="text-outline text-[9px] font-bold uppercase tracking-wider block mb-2">
              {stat.label}
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Scan Trend Chart */}
        <div className="bg-surface border border-outline-variant p-6 rounded-xl shadow-sm lg:col-span-2">
          <h4 className="text-xs font-bold text-on-surface uppercase tracking-wider mb-5 flex items-center gap-2">
            <svg
          ...
          <div className="flex items-end justify-between h-44 pt-4 border-b border-outline-variant px-2">
            {scanTrend.length === 0 ? (
              <div className="w-full h-full flex items-center justify-center text-on-surface-variant text-xs font-medium">
              ...
              scanTrend.map((t, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center flex-1 group relative">
                    ...
                    ></div>
                    <span className="text-[10px] text-outline font-bold uppercase tracking-wider mt-2.5">
                      {t.day}
                    </span>
                  </div>
                );
        ...
        <div className="bg-surface border border-outline-variant p-6 rounded-xl shadow-sm flex flex-col justify-between">
          <div>
          ...
          <div className="border-t border-outline-variant pt-4 mt-4 flex justify-between items-center text-xs">
            <span className="text-on-surface-variant font-semibold">Total Campaigns</span>
      ...
      <div id="campaigns" className="space-y-4 scroll-mt-6">
        <h3 className="text-lg font-bold text-on-surface">Active Loyalty Programs</h3>

        {campaigns.length === 0 ? (
          <div className="text-center py-16 bg-surface-container-lowest border border-outline-variant rounded-xl text-on-surface-variant text-sm shadow-sm font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {campaigns.map((camp) => (
              <div
                key={camp._id}
                className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 md:p-6 hover:shadow-sm hover:border-primary/30 transition-all duration-200 flex flex-col justify-between space-y-4 shadow-sm"
              >
                <div className="space-y-3.5">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-on-surface-variant font-bold bg-surface-container border border-outline-variant px-2.5 py-1 rounded-lg">
                        {camp.enrollmentCount || 0} enrolled
                      </span>
                      <button
                        onClick={() => handleDeleteCampaign(camp._id)}
                        className="p-1.5 rounded-lg text-outline hover:text-error hover:bg-red-50 border border-transparent hover:border-red-100 transition-all duration-200"
                  ...
                  <h4 className="text-base font-bold text-on-surface">{camp.title}</h4>
                  <p className="text-on-surface-variant text-xs leading-normal font-medium">{camp.description}</p>

                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-surface border border-outline-variant p-2.5 rounded-lg text-center">
                      <div className="text-sm font-bold text-primary">{camp.requiredStamps}</div>
                      <div className="text-[8px] text-outline font-bold uppercase tracking-wider mt-0.5">Stamps</div>
                    </div>
                    <div className="bg-surface border border-outline-variant p-2.5 rounded-lg text-center">
                      <div className="text-sm font-bold text-on-surface">{camp.pointsPerCheckin || 10}</div>
                      <div className="text-[8px] text-outline font-bold uppercase tracking-wider mt-0.5">Pts/Day</div>
                    </div>
                    <div className="bg-surface border border-outline-variant p-2.5 rounded-lg text-center">
                      <div className="text-sm font-bold text-secondary">✨</div>
                      <div className="text-[8px] text-outline font-bold uppercase tracking-wider mt-0.5">Reward</div>
                    </div>
                  </div>
                  ...
                  <div className="bg-primary/5 border border-primary/10 p-3 rounded-lg">
                    ...
                    <span className="text-xs font-semibold text-on-surface">🎁 {camp.rewardTitle}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-outline-variant">
                  <button
                    ...
                    className="flex-1 text-center py-2.5 rounded-lg bg-surface border border-outline-variant hover:border-outline text-on-surface font-bold text-[10px] transition-all uppercase tracking-wider"
                  >
                    Join QR
                  </button>
                  <button
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-outline-variant bg-surface-container-low flex items-center gap-2">
          ...
          <h4 className="text-xs font-bold text-on-surface uppercase tracking-wider">Recent Stamp Scan History</h4>
        </div>
                ...
                {recentStamps.map((stamp) => (
                  <tr key={stamp._id} className="hover:bg-surface-container-low transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-on-surface-variant mt-0.5">{stamp.customerId?.email}</div>
                    </td>
                    <td className="px-6 py-4 font-mono text-on-surface text-sm">{stamp.billNumber || "—"}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-[fade-in_0.2s_ease-out]">
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl max-w-lg w-full p-6 space-y-6 relative max-h-[90vh] overflow-y-auto shadow-2xl">
            <button
              onClick={() => setShowForm(false)}
              className="absolute top-4 right-4 text-outline hover:text-on-surface text-xl font-bold bg-surface border border-outline-variant hover:border-outline w-8 h-8 rounded-full flex items-center justify-center transition-colors"
            >
              ×
            </button>

            <div>
              <h3 className="text-xl font-bold text-on-surface">Configure New Campaign</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <label className="block text-outline text-[10px] font-bold uppercase tracking-wider">
                  Campaign Title
                </label>
                <input
                  type="text"
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
                  className="flex-1 py-3 bg-surface border border-outline-variant hover:border-outline text-on-surface-variant text-xs font-semibold rounded-lg transition-all uppercase tracking-wider"
                >
                  Cancel
                </button>
                <button
      ...
      {/* QR MODAL WORKSPACE */}
      {selectedCampaign && (
        <div className="fixed inset-0 bg-surface z-[100] flex flex-col animate-[fade-in_0.2s_ease-out]">
          {/* Top Header Row */}
          <div className="w-full border-b border-outline-variant py-4 px-6 bg-surface-container-lowest shrink-0">
            <div className="max-w-6xl mx-auto flex justify-between items-center gap-4">
              ...
              <button
                ...
                className="text-on-surface-variant hover:text-on-surface bg-surface border border-outline-variant px-4 py-2 rounded-lg text-xs font-semibold transition-all active:scale-95 whitespace-nowrap"
              >
                ✕ Close Workspace
              </button>
            </div>
          ...
          <div className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-6 flex flex-col md:flex-row gap-6 min-h-0 overflow-hidden">
            ...
            <div className="w-full md:w-64 shrink-0 flex flex-col gap-4">
              <div className="text-[10px] font-bold text-outline uppercase tracking-wider hidden md:block">
                Navigation
              </div>

              <div className="flex md:flex-col bg-surface-container rounded-xl md:rounded-none p-1 md:p-0 gap-1 border border-outline-variant md:border-0">
                <button
              ...
              {/* Guide card */}
              <div className="hidden md:block bg-surface border border-outline-variant rounded-lg p-4 mt-auto">
                <h4 className="text-[10px] font-bold text-outline uppercase tracking-wider mb-1.5">
                  Workspace Guide
                </h4>
                <p className="text-[10.5px] text-on-surface-variant leading-relaxed font-medium">
            ...
            <div className="flex-grow bg-surface border border-outline-variant rounded-xl p-6 overflow-y-auto min-h-0">
              {/* JOIN INVITE TAB */}
              {qrMode === "join" && (
                <div className="space-y-6 flex flex-col items-center py-2 animate-[fade-in_0.2s_ease-out]">
                  <div className="w-full max-w-sm bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm relative overflow-hidden group">
                    <div className="flex justify-between items-center mb-5 border-b border-dashed border-outline-variant pb-4">
                      <div>
                        <div className="text-[10px] font-bold text-outline uppercase tracking-wider font-mono">
                    ...
                    <div className="bg-surface border border-outline-variant p-5 rounded-lg w-fit mx-auto shadow-sm mb-5">
                      {joinQrDataUrl ? (
                        <img
                          src={joinQrDataUrl}
                          alt="Join QR Code"
                        ...
                        <div className="w-44 h-44 bg-surface rounded flex items-center justify-center text-outline text-xs animate-pulse">
                    ...
                    <div className="space-y-1.5 text-left">
                      <label className="block text-outline text-[9px] font-bold uppercase tracking-wider">
                        Invitation Link
                      </label>
                      <div className="flex gap-2 bg-surface border border-outline-variant rounded-lg p-1.5 shadow-sm">
                        <span className="text-[10px] font-mono text-on-surface-variant break-all select-all flex-grow py-1 px-2.5 overflow-hidden text-ellipsis whitespace-nowrap self-center">
                  ...
                  <div className="flex gap-3 w-full max-w-sm pt-1">
                    ...
                    <button
                      ...
                      className="flex-grow py-3.5 bg-surface border border-outline-variant hover:border-outline text-on-surface font-bold text-xs rounded-lg transition-all uppercase tracking-wider flex items-center justify-center gap-1.5"
                    >
                      🖨&nbsp; Print Card
                    </button>
                  </div>
                  ...
                  <div className="w-full max-w-sm bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="bg-surface border border-outline-variant p-5 rounded-lg w-fit mx-auto shadow-sm mb-5">
                      {dynamicQrDataUrl ? (
                        ...
                        <div className="w-44 h-44 bg-surface rounded flex items-center justify-center">
                    ...
                    <p className="text-[10px] text-on-surface-variant font-medium text-center leading-relaxed px-4">
                  ...
                  {!bulkGenerated ? (
                    <div className="w-full max-w-md mx-auto space-y-5">
                      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 space-y-5 shadow-sm">
                        <div className="flex items-center gap-3.5 border-b border-outline-variant pb-4">
                          ...
                          <div>
                            <div className="text-sm font-bold text-on-surface">Generate Batch Stamp Codes</div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-2 pt-2 border-t border-outline-variant mt-3">
                            ...
                            <input
                              ...
                              className="w-16 bg-surface border border-outline-variant rounded-lg py-1 px-2.5 text-xs text-center text-on-surface font-bold focus:outline-none focus:border-primary transition-colors"
                            />
                    ...
                    <div className="space-y-4">
                      {/* Summary toolbar */}
                      <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-4 flex flex-col sm:flex-row gap-3.5 justify-between items-center shadow-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="text-xs font-bold text-on-surface">Active Batch Sheets</div>
                            <div className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider mt-0.5">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto shrink-0">
                          ...
                          <button
                            onClick={printBulkQr}
                            className="flex-grow sm:flex-none py-2 px-3.5 bg-surface border border-outline-variant hover:border-outline text-on-surface font-semibold text-[10px] rounded-lg transition-all uppercase tracking-wider flex items-center justify-center gap-1.5 hover:scale-[1.01]"
                          >
                            🖨 Print Sheet
                          </button>
                          <button
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        {bulkQrCodes.map((qr) => (
                          <div
                            ...
                            className="bg-surface-container-lowest border border-outline-variant rounded-lg p-4 text-center hover:shadow-sm hover:border-primary/40 transition-all cursor-pointer flex flex-col items-center relative overflow-hidden shadow-sm"
                          >
                            {/* QR code image */}
                            <div className="bg-surface border border-outline-variant p-2 rounded-lg w-24 h-24 flex items-center justify-center transition-all shadow-inner mb-3">
                              <img
                                src={qr.dataUrl}
                                alt={`QR #${qr.index}`}
                            ...
                            <div className="w-full relative my-1.5">
                              <div className="absolute -left-[22px] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-surface border-r border-outline-variant z-10"></div>
                              <div className="absolute -right-[22px] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-surface border-l border-outline-variant z-10"></div>
                              <div className="w-full border-b border-dashed border-outline-variant"></div>
                            </div>
                            ...
                            <div className="w-full space-y-1 mt-1">
                              ...
                              <div className="text-[8px] text-outline font-bold uppercase tracking-wider">
                                Single stamp
                              </div>
                            </div>
                          </div>
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-4 animate-[fade-in_0.2s_ease-out]">
          <div className="bg-surface border border-outline-variant rounded-xl max-w-sm w-full p-6 text-center space-y-5 relative shadow-2xl">
            ...
            <div className="flex gap-3 pt-1">
              <button
                ...
                className="flex-1 py-3 bg-surface border border-outline-variant text-on-surface-variant font-semibold text-xs rounded-lg transition-all uppercase tracking-wider"
              >
                Cancel
              </button>
              <button
      ...
      {alertModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-4 animate-[fade-in_0.2s_ease-out]">
          <div className="bg-surface border border-outline-variant rounded-xl max-w-sm w-full p-6 text-center space-y-5 relative shadow-2xl">
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-on-surface">{alertModal.title}</h3>
              <p className="text-xs text-on-surface-variant leading-relaxed font-medium">
                {alertModal.message}
              </p>
            </div>
      ...
      {selectedZoomQr && (
        <div
          className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-4 animate-[fade-in_0.2s_ease-out]"
          onClick={() => setSelectedZoomQr(null)}
        >
          <div
            className="bg-surface border border-outline-variant rounded-xl max-w-sm w-full p-6 text-center space-y-4 relative shadow-2xl"
          ...
          >
            ...
            <div className="space-y-1">
              <h3 className="text-base font-bold text-on-surface">Scan QR #{selectedZoomQr.index}</h3>
              <p className="text-xs text-on-surface-variant font-medium">Single-use check-in code</p>
            </div>

            <div className="bg-surface border border-outline-variant p-5 rounded-lg w-fit mx-auto shadow-sm mb-2">
              <img
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                onClick={() => setSelectedZoomQr(null)}
                className="flex-grow py-2.5 bg-surface border border-outline-variant hover:border-outline text-on-surface-variant font-bold text-[10px] rounded-lg transition-all uppercase tracking-wider"
              >
                Close
              </button>
            </div>
